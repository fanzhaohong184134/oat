...
namespace Wit.SDK.Modular.WitSensorApi.Modular.BWT901BLE
{
    ...
    public class Bwt901ble : IAttitudeSensorApi
    {
        ...
        /// 寄存器0x33: 毫秒
        /// </summary>
        public void SetChipTime(int year, int month, int day, int hour, int minute, int second, int millisecond)
        {
            ushort reg30 = (ushort)((year - 2000) | (month << 8));
            ushort reg31 = (ushort)(day | (hour << 8));
            ushort reg32 = (ushort)(minute | (second << 8));
            ushort reg33 = (ushort)millisecond;

            SendProtocolData(new byte[] { 0xff, 0xaa, 0x30, (byte)(reg30 & 0xFF), (byte)(reg30 >> 8) });
            SendProtocolData(new byte[] { 0xff, 0xaa, 0x31, (byte)(reg31 & 0xFF), (byte)(reg31 >> 8) });
            SendProtocolData(new byte[] { 0xff, 0xaa, 0x32, (byte)(reg32 & 0xFF), (byte)(reg32 >> 8) });
            SendProtocolData(new byte[] { 0xff, 0xaa, 0x33, (byte)(reg33 & 0xFF), (byte)(reg33 >> 8) });
        }
